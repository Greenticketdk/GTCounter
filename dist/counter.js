...
(function (GT) {
    GT.Views.Countdown = function (options) {
        ...
        var reload = function () {
            ...
            secondText.draw(Math.floor(seconds));
        };
        ...
        var numberx = counterx - (canvas.width / 2);
        ...
        var minuteText = new CountdownNumber(this.ctx, numberx, countery - 40, Math.sqrt(area) / 7, this.settings.minText);
        var secondText = new CountdownNumber(this.ctx, numberx, countery + 40, Math.sqrt(area) / 7, this.settings.secText);
        var self = this;
    };
})(GT);
